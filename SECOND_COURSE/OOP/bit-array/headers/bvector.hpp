...
namespace bits
{
  ...
  public:
    ...
    constexpr size_type max_size() const noexcept { return SIZE_MAX; }
    ...
    constexpr size_type count() const noexcept
    {
      ...
      {
        byte = ((this->storage[i] >> 1) & 0b1010101) +
                (this->storage[i] & 0b1010101);
        byte = ((byte >> 2) & 0b11001100) + (byte & 0b11001100);
        byte = ((byte >> 4) & 0b11110000) + (byte & 0b11110000);
        bit_count += byte;
      }*/

      for (size_type i {}; i < this->bits; ++i)
      {
        if (this->storage[i >> 3] & 0b10000000 >> (i & 0b00000111)) { ++bit_count; }
      }

      /*for (size_type i { this->bits - (this->bits & 0b00000111)}; i < this->bits; ++i)
      {
        if (this->storage[i >> 3] & (0b10000000 >> (i & 0b00000111))) { ++bit_count; }
      }*/
    ...
    void resize(size_type num_bits, bool value = false)
    {
      if (num_bits < 0)
      {
        throw std::invalid_argument ("bvector:resize() -> invalid number of bits");
      }
      else if (num_bits == 0) { this->clear(); return; }
    ...
    {
      if (num_bytes > 268'435'456)
      {
        throw std::length_error ("bvector:reserve() -> invalid number of bytes");
      }
      else if (num_bytes == 0 || this->bytes == 268'435'456) { return; }
    ...
    {
      if (this->bytes && this->bytes < 268'435'456)
      {
        ...
        if (tmp_capacity > this->bytes)
        {
          if (this->bytes < 67'108'864)
          {
            tmp_capacity = this->bytes;
          }
          else if (!(this->bytes + (tmp_capacity = this->bytes >> 1) < 268'435'456))
          {
            tmp_capacity = 268'435'456 - this->bytes;
          }
      ...
      else if (this->bytes)
      {
        ...
        if (this->bits < SIZE_MAX) { ++this->bits; }
      }
      else
      {
        try { this->reserve(1); }
    ...
    bool pop_back()
    {
      ...
      return this->storage[this->bits / 8] &
            (0b10000000 >> (this->bits-- & 0b00000111));
    }

    bvector &set(i64 index, bool value = true)
    {
